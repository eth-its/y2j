...
uninstall() {
    ...
    if rm -f "${target}/y2j.sh" "${target}/y2j" "${target}/j2y" "${target}/yq"; then
        echo "Uninstalled ${target}/{y2j.sh,y2j,j2y,yq}."
    else
...
for doc in yaml.load_all(sys.stdin, Loader=yaml.SafeLoader):
...
y2j_sh() {
    ...
    shift 1
    case "$cmd" in
        installer|uninstall|version|j2y|y2j|yq)
            "$cmd" "$@"
        ;;
        *)
            usage
